class Api::V1::SwapperHostConnectionsController < Api::V1::ApiController
  ...
  def notify_host_on_cancel_request
    ...
    if PushNotificationService.notify_host_on_cancel_request(@user).present?
      Notification.create(subject: "Cancel Connection Request", body: "Request has been cancelled by Swapper.", notify_by: "Swapper", swapper_id: connection.swapper_id, host_id: connection.host_id)
  ...
  def notify_swapper_for_confirm_arrival
    ...
    if PushNotificationService.notify_swapper_for_confirm_arrival(@user).present?
      Notification.create(subject: "Confirm Arrival", body: "Host is issuing a Confirm Arrival, are you still interested in the spot?.", notify_by: "Host", swapper_id: connection.swapper_id, host_id: connection.host_id)
  ...
  def notify_host_swapper_is_still_interested
    ...
    if PushNotificationService.notify_host_swapper_is_still_interested(connection).present?
      Notification.create(subject: "Still Interested in Slot", body: "Swapper #{connection.swapper.name} is still interested in your Parking Slot.", notify_by: "Swapper", swapper_id: connection.swapper_id, host_id: connection.host_id)
      render json: {message: "Notification has been sent successfully to the Host."}, status: :ok
    else
      render json: {error: "Notification could not be sent."}, status: :unprocessable_entity
    end
